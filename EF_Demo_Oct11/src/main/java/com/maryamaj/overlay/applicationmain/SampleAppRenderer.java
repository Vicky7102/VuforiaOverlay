...
public class SampleAppRenderer {
    ...
    private boolean videoRenderPaused = false;
    private SparseArray<Mesh> vbMeshes;
    private SparseArray<float[]> vbProjectionMatrices;
    ...
    public SampleAppRenderer(SampleAppRendererControl renderingInterface, Activity activity, int deviceMode,
        ...
        vbMeshes = new SparseArray<>();
        vbProjectionMatrices = new SparseArray<>();
    ...
    public boolean isVideoRenderPaused() {
        return videoRenderPaused;
    }

    public void setVideoRenderPaused(boolean videoRenderPaused) {
        this.videoRenderPaused = videoRenderPaused;
    }
    ...
    public void renderVideoBackground() {
        ...
        if(!videoRenderPaused) {
            if (!mRenderer.updateVideoBackgroundTexture(videoBackgroundTex)) {
                Log.e(LOGTAG, "Unable to update video background texture");
                return;
            }
            vbProjectionMatrices.put(currentView, Tool.convert2GLMatrix(mRenderingPrimitives.getVideoBackgroundProjectionMatrix(currentView, COORDINATE_SYSTEM_TYPE.COORDINATE_SYSTEM_CAMERA)).getData());
        }
        float[] vbProjectionMatrix = vbProjectionMatrices.get(currentView);
        ...
        if (!videoRenderPaused) {
            vbMeshes.put(currentView, mRenderingPrimitives.getVideoBackgroundMesh(currentView));
        }
        Mesh vbMesh = vbMeshes.get(currentView);
